...
# cryptography is only needed to DECRYPT password/AES-protected PDFs. Its
# native _rust extension can fail to load on machines missing the VC++ runtime
# ("DLL load failed while importing _rust"). Import it lazily so that reading
# normal (unencrypted) PDFs never depends on the native module. Encrypted PDFs
# will raise a clear error only if actually encountered.
try:
    from cryptography.hazmat.backends import default_backend
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
except Exception:  # pragma: no cover - environment without working cryptography
