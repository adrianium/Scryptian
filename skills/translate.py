...
def run(text):
    ...
    """
    tl = _get_lang_code()
    key = f"{tl}:{(text or '').strip()}"
    if key in _CACHE:
        return _CACHE[key]

    url = "https://translate.googleapis.com/translate_a/single"
    params = parse.urlencode({"client": "gtx", "sl": "auto", "tl": tl, "dt": "t", "q": text})
    req = request.Request(f"{url}?{params}", headers={"User-Agent": "Mozilla/5.0"})
    ...
    result = bridge.generate(f"Translate the following text to {tl}. Output ONLY the translated text:\n\n{text}")
    _CACHE[key] = result
    return result
